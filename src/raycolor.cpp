...
double EPSILON_FACTOR_REFLECT = 0.0001;
  ...
  Eigen::Vector3d & rgb)
{
    ...
    Eigen::Vector3d rgb_phong, light_mirrored, rgb_mirrored, dir_light;
    Ray reflected_ray;
    ...
    if( first_hit(ray, min_t, objects, hit_id, t, n) ){
        ...
        rgb_mirrored = Eigen::Vector3d::Zero();
        ...
        if(! (objects[hit_id]->material->km == Eigen::Vector3d::Zero()) ){
            ...
            if(num_recursive_calls < MAX_RECURSIONS){
                ...
                if(raycolor(reflected_ray, 0.0, objects, lights, num_recursive_calls + 1, light_mirrored)){
